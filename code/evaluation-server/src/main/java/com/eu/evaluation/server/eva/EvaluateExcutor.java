...
public class EvaluateExcutor {
     ...
     * @throws Exception
     */
    public void execute(EvaluateVersion ev) throws ClassNotFoundException, InstantiationException, IllegalAccessException, Exception {
        ...
        //复制评测项目到历史信息
        initEvaluateItem(ev);
        logger.info("已完成评测项目初始化，即将开始评测结果初始化");
        ...
        //TODO 整理评测结果
        ResultCollator collator = new ResultCollator();
        collator.collate(ev);
     ...
     * @param ev
     */
    private void initData(EvaluateVersion ev) {
        dataService.copyData(ev);
    }
     ...
     */
    private void initResult(String evaluateVersionID) {
        ...
        for (EvaluateItemHistory item : itemHistorys) {//此处可考虑多线程调用
            ...
            List evaluatedEntity = evaluateService.findEvaluatedEntity(item);
            for (Object obj : evaluatedEntity) {
                ...
                result.setInstanceType(entity.getEntityType().getIndex());
     ...
     * @throws Exception
     */
    private void execute(String evaluateVersionID) throws InstantiationException, IllegalAccessException, Exception {
        EvaluateVersion ev = evaluateService.findEvaluateVersionByID(evaluateVersionID);
        
        //查找未执行的评测
        List<Result> unFinishedItem = evaluateService.findUnFinishedEvaluate(ev);
     ...
     * @throws Exception
     */
    private void execute(Result result) throws ClassNotFoundException, InstantiationException, IllegalAccessException, Exception {
        //实例化评测逻辑实现类
        Evaluating evaluating = EvaluatingFactory.getEvaluating(result.getItemHistory().getEvaluateClass());

        //对每个数据执行评测逻辑
        boolean passed = evaluating.evaluate(result.getItemHistory().getId(), result.getInstanceClass() , result.getInstanceType() , result.getInstanceId());
        if (!passed) {
